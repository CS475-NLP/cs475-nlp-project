...
# Pretraining dataset:
# We look at 7 topics.
# Filter articles that have (one or more of those 7 topics) + (nonzero number of additional topics).
# Exclude those that are used in actual training (articles with exactly 1 out of 7 topics).
# Label these articles as normal.
#


class Pretrain_Reuters_Dataset(TorchnlpDataset):
    ...
    def __init__(self, root: str, normal_class=0, tokenizer='spacy', use_tfidf_weights=False, append_sos=False,
        ...
        self.normal_classes = classes # 모든 class가 normal이라고 설
        del classes[normal_class]
        self.outlier_classes = classes

        # Load the reuters dataset (Test는 필요없음)
        self.train_set, self.test_set = reuters_dataset(directory=root, train=True, test=False, clean_txt=clean_txt)
